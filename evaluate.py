...
def plot_img(img_arr, img_name_arr, name, idx, title, save_img=False, cmap='gray'):
    ...
    def plot_ax(ax, im, sub_title):
        h = ax.imshow(im, cmap=cmap,vmin=0, vmax=1)
        ax.autoscale(False)
    ...
    ax = plt.subplot("222")
    plot_ax(ax, img_arr[1], '%s, rrmse = %.4f, ssim = %.4f'% (img_name_arr[1], rrmse_arr[1], ssim_arr[1]))
    ax = plt.subplot("223")
    plot_ax(ax, img_arr[2], '%s, rrmse = %.4f, ssim = %.4f'% (img_name_arr[2], rrmse_arr[2], ssim_arr[2]))
    ax = plt.subplot("224")
    plot_ax(ax, img_arr[3], '%s, rrmse = %.4f, ssim = %.4f'% (img_name_arr[3], rrmse_arr[3], ssim_arr[3]))
    # fig.suptitle(title)
    if save_img:
...
def create_rrmse_data(corr_type, model_type, plot_title, gpu_id):
    print('\n------------------------')
...
# %%
gpu_id = 1
create_rrmse_data('low_1', 'Undersample + Noise low', gpu_id=gpu_id)

# %%
gpu_id = 1
create_rrmse_data('high_1', 'Undersample + Noise high', gpu_id=gpu_id)
create_rrmse_data('low_1', 'Undersample + Noise low', gpu_id=gpu_id)
create_rrmse_data('high_2', 'Noise high', gpu_id=gpu_id)
create_rrmse_data('low_2', 'Noise low', gpu_id=gpu_id)
create_rrmse_data('high_3', 'Undersample high', gpu_id=gpu_id)
create_rrmse_data('low_3', 'Undersample low', gpu_id=gpu_id)
