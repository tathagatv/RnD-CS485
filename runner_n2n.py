...
for i in range(train_img.shape[0]):
    train_X[i], train_X_spec[i] = corrupt_data_gaussian(0, train_spec[i], corrupt_params_gaussian)
    train_Y[i], train_Y_spec[i] = corrupt_data_gaussian(0, train_spec[i], corrupt_params_gaussian)

for i in range(test_img.shape[0]):
    test_X[i], test_X_spec[i] = corrupt_data_gaussian(0, test_spec[i], corrupt_params_gaussian)
    test_Y[i], test_Y_spec[i] = corrupt_data_gaussian(0, test_spec[i], corrupt_params_gaussian)
...
for ep in range(epochs):
    ...
    rrmse_history.append(rrmse_train)
